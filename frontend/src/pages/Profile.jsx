  ...
  function Profile() {
    ...
    const handleAddressSubmit = async (e) => {
      ...
      const isDuplicate = addresses.some((addr) => 
        normalized(addr.city) === normalized(addressForm.city) &&
        normalized(addr.street) === normalized(addressForm.street) &&
        normalized(addr.house) === normalized(addressForm.house) &&
        normalized(addr.building) === normalized(addressForm.building) &&
        normalized(addr.apartment) === normalized(addressForm.apartment) &&
        normalized(addr.postal_code) === normalized(addressForm.postal_code) &&
        addr.id !== editAddressId
      );
    
      if (isDuplicate) {
        setError('Такой адрес уже существует');
        return;
      }
